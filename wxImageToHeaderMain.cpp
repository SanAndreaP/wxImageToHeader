...
void wxImageToHeaderFrame::OnCB_FileOutClick(wxCommandEvent& event)
{
    if( this->CB_FileOut->GetValue() ) {
        ...
        this->Btn_FileOut->Enable();
        this->SetSize(wxSize(598, this->GetSize().GetHeight()));
    } else {
        this->TC_FileOut->Disable();
        this->Btn_FileOut->Disable();
        this->SetSize(wxSize(993, this->GetSize().GetHeight()));
    }
}
